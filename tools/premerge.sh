...
if [ "$full" -eq 1 ]; then
  step "1/12 full suite (unit + integration)"
  mvn -B -q -T 1C $REPO_ARGS -Pintegration verify \
    || { echo "FAIL: mvn -Pintegration verify"; fail=1; }
else
  step "1/12 build (suites left to CI; --full runs them here)"
...
step "2/12 model harness vs baseline"
...
step "3/12 chart harness vs baseline"
...
step "4/12 lyric harness vs baseline"
...
step "5/12 transcription harness vs baseline"
...
step "6/12 synthetic harness vs baseline"
...
step "7/12 melody harness vs baseline"
...
# The page a solo instrument gets (#814): the same accompaniment-free packages,
# taken through the playable part and scored on the grid rather than in
# seconds, so the beat, the key and the notes are read off what was engraved.
# Committed corpus, so CI runs this diff too.
step "8/12 solo harness vs baseline"
compare score-solo.py tools/baselines/score-solo.txt || { echo "FAIL: score-solo moved — if intended, regenerate the baseline and commit it"; fail=1; }
...
# docstring.
step "10/12 melody harness vs baseline, on real singing"
...
step "11/12 melody harness vs baseline, through the separated vocal"
...
step "12/12 melody harness vs baseline, real singing through the separated vocal"
