 ...
 * of a short line, exactly as {@link StaffNotation} always has, and
 * {@code ChordChartTest.noEmittedLineBeginsWithABarCheck} fails if that stops
 * being so. It is reachable from a <em>title</em>: neither emitter escapes a
 * newline, so a title containing one splits a header line and can start the
 * remainder with anything at all. No shipped path can carry such a title, and
 * #192 tracks closing it. #169 tracks the remainder.
 *
