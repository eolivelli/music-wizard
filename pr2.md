...
`medianTempo` is the only place in the reactor that converts a *`BeatGrid`'s*
pulses to quarter notes — `TempoMap.constantPulse` and `fromBeatTimes` convert a
meter's counted pulses, which is a different question. Its only main-source
caller is `Score.estimatedTempo()`. The other
