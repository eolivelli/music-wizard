...
take engraves as played, and so does the flute take once its note values
...
`StaffLayout`, `ChartLayout` and `LyricEngraving` decisions. The chart is
rests under chord symbols; the lyric sheet hides its rests and hangs the words
on them. Not carried: beat marks and repeat tags (#777); chords and words past
the staff's last bar on the lead sheet, which the `.ly` prints in its running
chord context (#778); an extender ended by a syllable said as text (#789); a
chart bar longer than its staff bar is refused rather than misplaced (#787). A
held chord reprinted after a system break on the page is unlabelled in the
document.
