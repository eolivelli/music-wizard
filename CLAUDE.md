...
grid written down there and `BluesLoopIT` gates one recording in CI. A solo
take carries its melody in a `.melody.txt` beside it instead of a grid, and
`tools/score-solo.py --source samples` scores the page engraved from it.
