...
that does not hold one (#187). Then: the three-eighths tempo lock
is gone — the estimator weighs candidates by whether the harmony can be barred
by them (#231) — leaving that benchmark at a plain half-tempo octave, which is
`analyze --tempo`'s to double. And the vocabulary still has no major seventh,
